...
struct list_node* list_end(struct list_node *head)
{
    ...
    {
        // once (*head).next is NULL, it's the end of the linked list
        return head;
    }
}
...
struct list_node * list_find_helper(struct list_node *head, int value, struct list_node **predp)
{
    if ((*head).next == NULL)
    {
        *predp = NULL;
    ...
    {
        return list_find_helper((*head).next, value, predp);
    }
}

struct list_node * list_find(struct list_node *head, int value, struct list_node **predp)
{
    // find is for the exception that the head contains value, otherwise
    // calls a helper function that recurses to find value
    assert(head != NULL);
    assert(predp != NULL);
    if ((*head).value == value)
    {
    ...
    {
        return list_find_helper(head, value, predp);
    }
}
...
int list_remove(struct list_node **headp, int value)
{
    assert(headp != NULL);
    assert(*headp != NULL);
    struct list_node *before_node;
    struct list_node *removed_node = list_find(*headp, value, &before_node);
    if (removed_node == NULL)
    {
        // case where you don't remove anything
        return 0;
    }
    if (before_node == NULL)
    {
        // case where value is at the head
        *headp = (*removed_node).next;
    }
    else
    {
        if ((*removed_node).next == NULL)
        {
            // case where value is at the end
            (*before_node).next = NULL;
        }
        else
        {
            // "regular" case where value is in the middle
            (*before_node).next = (*removed_node).next;
        }
    }
    // always do these when freeing
